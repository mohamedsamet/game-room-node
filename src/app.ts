...
import { socketListenServer } from './web/socket/socket.server';
import mongoose from 'mongoose';
import { EXPRESS_API_PORT } from './constants/api.constant';
import {
...
mongoose.connect(`${MONGO_FINAL_URL}`, MONGO_CONNECTION_OPTIONS, (error) => {
  if (!error) {
    ...
    const appServer = expressServer.listen(EXPRESS_API_PORT, () => {
      return console.log(`${EXPRESS_CONNECTED_LOG}${EXPRESS_API_PORT}`);
    });
    const io = socket(appServer);
    socketListenServer.socketListen(io);
  } else {
    console.error(error);
  }
});
